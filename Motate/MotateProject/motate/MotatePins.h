...
#pragma mark IRQPin / LookupIRQPin
    ...
    struct IRQPin : Pin<pinNum> {
        static_assert(Pin<pinNum>::isNull() || !IsIRQPin<pinNum>(), "假IRQPin只应位于空引脚或非IRQ引脚上。");
        ...
        static void interrupt();//xzw168
        void setInterruptHandler(std::function<void(void)> &&handler) {};
    };

    // 这是真正的IRQPin定义，仅用于引脚是真正定义的。
	#if 0    template<pin_number pinNum>
