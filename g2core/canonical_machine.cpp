/*
 * canonical_machine.cpp - rs274 / ngc规范机器.
...
cmMachine_t *cm;  // 指向活动规范机器的指针
cmMachine_t cm1;  // 规范机器初级
cmMachine_t cm2;  // 规范机器二次
cmToolTable_t tt; // 全局工具表
...
void cm_set_absolute_override(GCodeState_t *gcode_state, const uint8_t absolute_override)
{
    ...
    cm_set_display_offsets(MODEL); // 如果更改绝对覆盖，则必须重置偏移
}
...
void cm_set_model_target(const float target[], const bool flags[])
{
    ...
    memcpy(cm->gm.target, cm->gmx.position, sizeof(cm->gm.target));
    ...
    for (axis = AXIS_A; axis <= AXIS_C; axis++)
    {
        ...
        if (mst.marlin_flavor && (cm->a[axis].axis_mode == AXIS_RADIUS))
        {
            ...
            { // if (cm.gmx.extruder_mode == EXTRUDER_MOVES_NORMAL)
...
stat_t cm_set_tl_offset(const uint8_t H_word, const bool H_flag, const bool apply_additional)
{
    ...
    mp_queue_command(_exec_offset, value, nullptr_bool); // second vector (flags) is not used, so fake it
    return (STAT_OK);
}
...
stat_t cm_cancel_tl_offset()
{
    ...
    mp_queue_command(_exec_offset, value, FLAGS_ONE); // changes it in the runtime when executed
    return (STAT_OK);
}

stat_t cm_set_coord_system(const uint8_t coord_system) // 设置与planner同步的坐标系
{
    cm->gm.coord_system = (cmCoordSystem)coord_system;
    cm_set_display_offsets(MODEL); // 如果更改坐标系，则必须重置显示偏移

    float value[] = {(float)coord_system};
    mp_queue_command(_exec_offset, value, FLAGS_ONE); //xzw168
    return (STAT_OK);
}
...
stat_t cm_set_g92_offsets(const float offset[], const bool flag[])
{
    ...
    float value[] = {(float)cm->gm.coord_system}; // pass coordinate system in value[0] element
    mp_queue_command(_exec_offset, value, nullptr_bool);
    cm_set_display_offsets(MODEL);
    return (STAT_OK);
}
...
stat_t cm_straight_traverse(const float *target, const bool *flags, const uint8_t motion_profile)
{
    ...
    cm_set_model_target(target, flags);          //set cm->gm.target
    ritorno(cm_test_soft_limits(cm->gm.target)); // 测试软限制; 如果被抛出则退出
    cm_set_display_offsets(&cm->gm);             // 捕获完全解决的状态偏移  从模型中捕获组合偏移到有效Gcode动态模型中的绝对值
    cm_cycle_start();                            // 这里需要归位与其他周期
    stat_t status = mp_aline(&cm->gm);           // 将移动发送给计划者 计划加速/减速的线
	copy_vector(cm->gmx.position, cm->gm.target); //cm_update_model_position();                  // 更新gmx.position以准备下一次进入的移动 
	
    if (status == STAT_MINIMUM_LENGTH_MOVE)
    {                               //移动小于最小长度
        if (!mp->q.r->buffer_state) //if (!mp_has_runnable_buffer(mp))
        {                           // 处理零长度移动最后或仅移动的条件
            cm_cycle_end();         // ...否则循环不会正常结束 resets MACHINE_CYCLE
        }
        status = STAT_OK;
    }
    return (status);
...
stat_t cm_select_tool(const uint8_t tool_select)
{
    ...
    mp_queue_command(_exec_select_tool, value, nullptr_bool);
    return (STAT_OK);
}
...
stat_t cm_change_tool(const uint8_t tool_change)
{
    ...
    mp_queue_command(_exec_change_tool, value, nullptr_bool);
    return (STAT_OK);
}
...
stat_t cm_fro_control(const float P_word, const bool P_flag) // M50
{
    ...
    if (P_flag)
    { // if parameter is present in Gcode block
        if (fp_ZERO(P_word))
        {
...
{
    cmMachineState machine_state = (cmMachineState)((int)value[0]); //xzw168
...
void cm_cycle_start()
{
    ...
    {                                      // 不在归位，探测或其他固定循环中
        cm->cycle_type = CYCLE_MACHINING;  //在正常的加工循环中
        cm->machine_state = MACHINE_CYCLE; //机器处于循环，运行状态; 块仍然要运行，或者步进正忙
        qr_init_queue_report();            // 清除队列报告缓冲区计数//
    }
}
...
void cm_cycle_end()
{
    if (cm->cycle_type == CYCLE_MACHINING)
    {
        ...
        _exec_program_finalize(value, nullptr_bool);
    }
}
...
{
    float value[] = {(float)MACHINE_PROGRAM_STOP};
    mp_queue_command(_exec_program_finalize, value, nullptr_bool);
}
...
stat_t cm_set_jt(nvObj_t *nv)
{
    ...
    { // recalculate max_junction_accel now that time quanta has changed.
