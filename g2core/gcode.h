...
typedef enum
{                             // axis types. Enum must be in this order
...
{                      // axis modes (ordered: see _cm_get_feed_time())
    ...
    AXIS_STANDARD,     // 具有标准行为的协调运动轴
    AXIS_INHIBITED,    // 轴已计算但未激活
    AXIS_RADIUS        // 旋转轴校准到圆周
} cmAxisMode;
...
{                             // Gcode dynamic state extensions - used by model and arcs
    ...
    float position[AXES];     // XYZABC模型位置（注意：未在gn或gf中使用）
