...
typedef enum {
    SR_REQUEST_IMMEDIATE = 0,       // 请尽快请求完整或已过滤的状态报告（取决于SR_VERBOSITY设置）
    SR_REQUEST_IMMEDIATE_FULL,      // 请尽快申请完整状态报告（无论SR_VERBOSITY设置如何）
    SR_REQUEST_TIMED,               // 请求下一个计时器间隔的完整或过滤状态报告（如上所述）
    SR_REQUEST_TIMED_FULL           // 请求下一个计时器间隔的完整状态报告（如上所述）
...
typedef struct qrSingleton {        // data for queue reports
    ...
    uint8_t buffers_available;              // 存储的缓冲区深度由回调传递给
