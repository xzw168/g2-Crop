 ...
 * mp_aline() - 计划加速/减速的线
 *
 *此功能使用恒定的加加速度运动方程来计划加速度和减速度
*加加速度是加速度的变化率; 它是加速的第一个衍生物，
*和位置的第三衍生物。Jerk是对机器影响的衡量标准。
 *控制加加速度可平滑移动之间的过渡，并允许更快的进给
 *控制机器振荡和其他不良副作用。
 *
 *注意：所有数学都是使用单精度浮点（浮点数）以绝对坐标完成的。
 *
*注意：返回不是STAT_OK的状态意味着端点不是高级的。所以线条
 *太短而不能移动会累积并在累积错误后执行
 *超过最低要求。
 */
...
{
    float target_rotated[] = INIT_AXES_ZEROES; //目标旋转
    ...
    for (uint8_t axis = 0; axis < AXES; axis++)
    {
        ...
        if ((flags[axis] = fp_NOT_ZERO(axis_length[axis])))
        { // yes，这应该是= not ==
        ...
        {
            axis_length[axis] = 0; //如果它很小，那就真的为零
            axis_square[axis] = 0; //通过破坏_calculate_times中的block_time来修复可以杀死feedhold的bug
        }
    }
    ...
    if (length < 0.0001)
    {                                                    // 此值为0.1微米。防止计划者陷阱失败
        sr_request_status_report(SR_REQUEST_TIMED_FULL); // 是SR_REQUEST_IMMEDIATE_FULL
        return (STAT_MINIMUM_LENGTH_MOVE);               // 结束循环所需的STAT_MINIMUM_LENGTH_MOVE
    }
    ...
    copy_vector(bf->gm.target, target_rotated); //将旋转的目标复制到位

    // setup the buffer
    bf->bf_func = mp_exec_aline; //将回调注册到exec函数
    bf->length = length;         //记录长度
    for (uint8_t axis = 0; axis < AXES; axis++)
    { //计算单位矢量并设置标志
        if ((bf->axis_flags[axis] = flags[axis]))
        {                                                // yes，这应该是=而不是==
            bf->unit[axis] = axis_length[axis] / length; // nb：bf->单位被mp_get_write_buffer（）清除
        }
    }
    _calculate_jerk(bf);                             //计算bf-> jerk值
    _calculate_vmaxes(bf, axis_length, axis_square); // compute cruise_vmax and absolute_vmax
    _set_bf_diagnostics(bf);                         // DIAGNOSTIC

    //注意：这些下一行必须保持准确的顺序。在提交缓冲区之前必须更新位置。
    copy_vector(mp->position, bf->gm.target); //更新下一步的计划员位置
    mp_commit_write_buffer(BLOCK_TYPE_ALINE); //提交当前块（必须遵循位置更新）
    return (STAT_OK);
}

/****************************************************************************************
 * mp_plan_block_list() - 计划列表中的所有块
 *
...
/****************************************************************************************
 * _plan_block() - 将新块拼接并计划到规划器队列
 */

static mpBuf_t *_plan_block(mpBuf_t *bf)
{
	//第一时间块 - 为尽可能多的块设置vmax（向前加载引导块）
	//注意：cruise_vmax是在aline（）的_calculate_vmaxes（）中计算的
    if (mp->planner_state == PLANNER_PRIMING)
    {
        ...
        if (bf->pv->plannable)
        {
			// 计算最大出口速度可以通过交汇点。
			//bf->pv->junction_vmax
            _calculate_junction_vmax(bf->pv); 
        ...
        _calculate_override(bf); //计算cruise_vmax给定的cruise_vset和进给速率系数
        ...
        mp->planning_return = bf->nx;              // 计划完成后返回的地方
        mp->planner_state = PLANNER_BACK_PLANNING; // 开始计划
    }
...
/***** ALINE HELPERS *****
 * _calculate_override() - 计算cruise_vmax给定的cruise_vset和进给速率系数
 * _calculate_jerk()
 * _calculate_vmaxes()
...
static void _calculate_override(mpBuf_t *bf) // execute ramp to adjust cruise velocity
{
    ...
    if (mp->ramp_active)
    {
        ...
        if (mp->ramp_dvdt > 0)
        { // positive is an acceleration ramp
            ...
            if (bf->cruise_velocity > bf->absolute_vmax)
            { // test max cruise_velocity
                bf->cruise_velocity = bf->absolute_vmax;
                mp->ramp_active = false; // don't allow exceeding absolute_vmax
            }
        }
        else
        { // negative is deceleration ramp
            if (bf->override_factor < mp->ramp_target)
            {
                bf->override_factor = mp->ramp_target;
...
/****************************************************************************************
 * _calculate_junction_vmax() - 最大出口速度可以通过交汇点。
 *
*警告：此说明已过期，需要更新。
 *
 *通过查找不会达到的速度来计算最大允许结点速度
 *违反任何轴的加加速度值。
 *
 *为了达到这个目的，我们取两个动作的单位向量的差值
 角落的*，从矢量a到矢量b的点。这两者的单位向量
 *移动作为当前块（a_unit）和前一个块（b_unit）提供。
 *
 * Delta [i] =（b_unit [i] - a_unit [i]）（1）
 *
 *我们逐轴采用“单位速度”的差异来得到一个矢量
 *表示加速方向 - 可能是相反的方向
 *作为“a”矢量来实现减速。要获得实际加速度，
 *我们使用角速度（我们打算计算的）作为幅度。
 *
 *加速度[i] = UnitAccel [i] *速度[i]（2）
 *
 *因为我们需要加加速度值，这被定义为“加速度的变化率，
 *即，相对于时间的加速度的衍生物“（维基百科），我们需要
 *有一段时间，加速度的变化实际上由
 *物理学。这将为我们提供“应用”加速度变化的时间
 *为了获得逼真的混蛋。产生一个相当简单的公式：
 *
 * Jerk [i] =加速[i] /时间（3）
 *
 *现在我们可以计算给定角落的加加速度，我们需要知道最大值
 *速度，我们可以采取角落而不违反任何轴的那个混蛋。
 *让我们将公式（2）纳入公式（3），并使用求解速度
 *这个角落的已知max Jerk和UnitAccel：
 *
 * Velocity [i] =（Jerk [i] * Time）/ UnitAccel [i]（4）
 *
 *然后我们为每个轴计算（4），并使用最小（最有限）的结果或
 * vmax，以较小者为准。
 * /
/ *注1：
 * junction_integration_time是以分钟表示的积分时间量。
 *这大约是1 DDA时钟滴答的顺序，以将加加速度与加速度相结合。
 *这是一个非常小的数字，所以我们将JT乘以1,000,000进行输入和显示。
 *合理的JA因此介于0.10和2.0之间
 *
 *在公式4中，加加速度乘以1,000,000，JT除以1,000,000，
 *所以这些条款取消。
 */
