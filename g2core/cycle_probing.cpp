...
uint8_t cm_straight_probe(float target[], bool flags[], bool trip_sense, bool alarm_flag) 
{
    ...
    mp_queue_command(_motion_end_callback, nullptr_float, nullptr_bool);  // note: these args are ignored
    return (STAT_OK);
} 
...
static stat_t _probe_move(const float target[], const bool flags[])
{
    ...
    mp_queue_command(_motion_end_callback, nullptr_float, nullptr_bool); // the last two arguments are ignored anyway
    return (STAT_EAGAIN);
}
