...
#define MODEL (GCodeState_t *)&cm->gm   // 来自规范机器gm模型的绝对指针
#define RUNTIME (GCodeState_t *)&mr->gm // 运行时mm结构的绝对指针
#define ACTIVE_MODEL cm->am             // 活动模型指针由cm_set_motion_state（）维护
...
typedef enum
{                             // Note: MachineState signals if the machine is in cycle (5) or some other non-cycle state
    ...
    MACHINE_CYCLE,            // 机器处于循环，运行状态; 块仍然要运行，或者步进正忙
...
typedef enum
{
    CYCLE_NONE = 0,  // 不是在一个循环中
    CYCLE_MACHINING, // 在正常的加工循环中
    CYCLE_HOMING,    // 在归位周期
    CYCLE_PROBE,     // 在探测循环中
    CYCLE_JOG        // 在慢跑周期
...
typedef struct cmArc
{ // planner and runtime variables for arc generation
    magic_t magic_start;
    uint8_t run_state; // runtime state machine sequence

    float position[AXES]; // 累积运行时位置
...
typedef struct cmToolTable
{                                     // struct to keep a global tool table
