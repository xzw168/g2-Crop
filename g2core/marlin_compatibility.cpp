/*
 * marlin_compatibility.cpp - 支持marlin协议和gcode
...
bool _queue_next_temperature_commands()
{
    if (MarlinSetTempState::Idle != set_temp_state) {
        ...
        if (MarlinSetTempState::StartingUpdates == set_temp_state) {
            mp_queue_command(_marlin_start_temperature_updates, nullptr_float, nullptr_bool);
        ...
        if (MarlinSetTempState::StoppingUpdates == set_temp_state) {
            mp_queue_command(_marlin_end_temperature_updates, nullptr_float, nullptr_bool);
