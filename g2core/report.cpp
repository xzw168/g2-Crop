...
/*
 * sr_request_status_report() - 请求状态报告
 *
...
/*
 * qr_request_queue_report() - 请求队列报告
 *
 *  请求队列报告，并记录添加和删除的缓冲区
 *  自上一个init（通常在生成报告时重新启动）。
 */

void qr_request_queue_report(int8_t buffers)
{
    // 获取缓冲区深度和添加/删除计数
    qr.buffers_available = mp_get_planner_buffers(mp);// 运行队列中可用缓冲区的计数
    if (buffers > 0) {
